...
function findDistance(light, block, angle, rLen, start, shortest, closestBlock){
    var y = (block.position.y) - light.position.y,
        x = (block.position.x) - light.position.x,
        dist = Math.sqrt((y * y) + (x * x));
        start = start

    if(light.radius >= dist) {
        var rads = angle * (Math.PI / 180);
        var pointPos = new vector(light.position.x, light.position.y);
...
function draw(){
    ...
    for(var i = 0; i < lights.length; i++){
        ...
        if(i==0) {
            lights[i].position.x = me.x / tileSize
            lights[i].position.y = me.y / tileSize
    ...
    canvas_main_ctx.drawImage(canvas_lighting, 0, 0, canvas_lighting.width, canvas_lighting.height, 0,0,canvas_main.width, canvas_main.height)
...
//player
lights.push(new light(new vector(256,256), 100, 120, 'rgba(255,255,255,.1)'));

function initRayCaster() {
    for(var i = 0; i < walls.length; i++){
        var size = 1
        blocks.push(new block(new vector(walls[i].x,walls[i].y),size,size));
    }
}